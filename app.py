...
COLUMN_CONFIG = {
    ...
    "Region": "Region",
    "Soil_Type": "Soil_Type",
    "Temperature": "Temperature_Celsius",  # 修正为真实列名
    "Rainfall": "Rainfall_mm",            # 修正为真实列名
    "Fertilizer_Used": "Fertilizer_Used",
    "Irrigation_Used": "Irrigation_Used",
    "Yield": "Yield_tons_per_hectare"     # 修正为真实列名
}
...
menu = st.sidebar.radio(
    "导航栏",
    options=["数据概览", "单产预测", "作物推荐", "管理方案推荐", "模型可解释性", "敏感性分析"]
)
...
if menu == "数据概览":
    ...
    # 输入表单
    st.write("### 请输入条件：")
    crop = st.selectbox("作物 (Crop)", ["Wheat", "Corn", "Rice", "Barley"])
    region = st.selectbox("区域 (Region)", ["North", "South", "East", "West"])
    soil_type = st.selectbox("土壤 (Soil_Type)", ["Loamy", "Clayey", "Sandy"])
    temperature = st.slider("温度 (Temperature)", min_value=10.0, max_value=40.0, step=0.5)
    rainfall = st.slider("降雨量 (Rainfall)", min_value=0.0, max_value=300.0, step=10.0)
    fertilizer_used = st.selectbox("是否施肥 (Fertilizer_Used)", [0, 1])
    irrigation_used = st.selectbox("是否灌溉 (Irrigation_Used)", [0, 1])
    weather_cond = st.selectbox("天气状况 (Weather_Condition)", ["Sunny", "Cloudy", "Rainy"])
    ...
    user_input = {
        ...
        "Region": region,
        "Soil_Type": soil_type,
        "Temperature_Celsius": temperature,  # 修正列名
        "Rainfall_mm": rainfall,            # 修正列名
        "Fertilizer_Used": fertilizer_used,
        "Irrigation_Used": irrigation_used,
    ...
    fertilizer_used = st.selectbox("是否施肥 (Fertilizer_Used)", [0, 1])
    irrigation_used = st.selectbox("是否灌溉 (Irrigation_Used)", [0, 1])
    weather_cond = st.selectbox("天气状况 (Weather_Condition)", ["Sunny", "Cloudy", "Rainy"])  # 确保包含 Weather_Condition
    ...
    if st.button("推荐最适合作物"):
        ...
        base_input = {
            ...
            "Fertilizer_Used": fertilizer_used,
            "Irrigation_Used": irrigation_used,
            "Weather_Condition": weather_cond   # 确保包含 Weather_Condition
        }
    ...
    # 用户输入地块条件
    st.write("### 请输入地块条件：")
    crop = st.selectbox("作物", ["Wheat", "Corn", "Rice", "Barley"])  # 新增：作物选择
    region = st.selectbox("区域", ["North", "South", "East", "West"])
    soil_type = st.selectbox("土壤 (Soil_Type)", ["Loamy", "Clayey", "Sandy"])
    temperature = st.slider("温度 (Temperature_Celsius)", min_value=10.0, max_value=40.0, step=0.5)
    rainfall = st.slider("降雨量 (Rainfall_mm)", min_value=0.0, max_value=300.0, step=10.0)
    weather_cond = st.selectbox("天气状况 (Weather_Condition)", ["Sunny", "Cloudy", "Rainy"])  # 新增：天气选择

    # 构造用户基础输入（注意列名必须和训练数据完全一致）
    base_input = {
        "Crop": crop,                             # 新增
        "Region": region,
        "Soil_Type": soil_type,
        "Temperature_Celsius": temperature,       # 修正列名
        "Rainfall_mm": rainfall,                  # 修正列名
        "Weather_Condition": weather_cond         # 新增
    }
    ...
    crop = st.selectbox("作物 (Crop)", ["Wheat", "Corn", "Rice", "Barley"])
    region = st.selectbox("区域 (Region)", ["North", "South", "East", "West"])
    soil_type = st.selectbox("土壤类型 (Soil_Type)", ["Loamy", "Clayey", "Sandy"])
    fertilizer_used = st.selectbox("是否施肥 (Fertilizer_Used)", [0, 1])
    irrigation_used = st.selectbox("是否灌溉 (Irrigation_Used)", [0, 1])
    weather_cond = st.selectbox("天气状况 (Weather_Condition)", ["Sunny", "Cloudy", "Rainy"])
    ...
    if sensitivity_var == "Temperature (温度)":
        sensitivity_range = st.slider("选择温度变化范围(°C)", 10.0, 40.0, (15.0, 35.0))
    else:
        sensitivity_range = st.slider("选择降雨量变化范围", 0.0, 300.0, (50.0, 250.0))
